...
export default function AdminDashboard() {
  ...
  const [depositPercent, setDepositPercent] = useState(20); // Default 20% down
  ...
  const handleCreateInvoice = async (e: React.FormEvent) => {
    ...
    // Automatically build clean structural breakdown text for legacy text view fallback
    const summaryText = lineItems.map(i => `**${i.title}** - $${i.cost}\n${i.description}`).join("\n\n");

    // Standard structural phase breakdown based on milestone progress percentages
    const defaultPhases = [
    ...
    <div className="min-h-screen bg-slate-100 py-12 px-4 font-sans text-gray-900">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="bg-white shadow-xl rounded-lg border border-gray-200 p-8">
          <h1 className="text-2xl font-bold border-b pb-4 mb-6">Contract Configuration Dashboard</h1>
          ...
          <form onSubmit={handleCreateInvoice} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
              <div className="sm:col-span-2 p-4 bg-blue-50/50 border border-blue-200 rounded-md flex gap-2">
                <input type="text" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} placeholder="Describe work (e.g., Basement remodel, black ceiling, 3pc bath)" className="flex-1 p-2.5 border rounded-md text-sm bg-white" />
                <button type="button" onClick={handleAiGenerate} disabled={isGeneratingAI} className="bg-blue-600 hover:bg-blue-700 text-white text-xs px-4 rounded-md font-medium transition whitespace-nowrap">
                  {isGeneratingAI ? "Processing..." : "Generate Proposal"}
                </button>
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Down Deposit (%)</label>
                <input type="number" min="5" max="50" value={depositPercent} onChange={(e) => setDepositPercent(parseInt(e.target.value) || 10)} className="w-full p-2.5 border rounded-md text-sm bg-white" />
              </div>
            </div>
            ...
            {lineItems.length > 0 && (
              <div className="border border-gray-200 rounded-lg overflow-hidden bg-white">
                ...
                <div className="bg-slate-900 text-white p-4 flex justify-between font-bold text-sm">
                  <span>TOTAL APPRAISED OUTLINE:</span>
            ...
            <button type="submit" disabled={isSubmitting} className="w-full bg-slate-950 hover:bg-slate-800 text-white font-semibold py-3 rounded-md text-sm shadow-md transition">
              {isSubmitting ? "Publishing Contract..." : "Deploy Active Proposal Link"}
            </button>
          </form>
        </div>
        ...
        {generatedLink && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-6 flex justify-between items-center shadow-md">
            <div>
              <h3 className="text-sm font-bold text-emerald-900">Proposal Deployed Successfully!</h3>

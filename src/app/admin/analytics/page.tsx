    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-sans text-[10px] uppercase tracking-architect text-taupe-400">Loading analytics</p>
        </div>
      </div>
    );
    ...
    <div className="pb-28 text-left">
      ...
      <div className="sticky top-0 z-20 border-b border-taupe-200/70 bg-alabaster-50/85 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:gap-6 sm:px-8 sm:py-5">
          ...
          <div className="grid shrink-0 grid-cols-4 gap-px overflow-hidden rounded-edge bg-taupe-200 shadow-riser ring-1 ring-taupe-200/60 sm:flex">
            {([["today", "Today"], ["7d", "7 Days"], ["30d", "30 Days"], ["all", "All"]] as const).map(([val, label]) => (
              <button
                ...
                className={`px-2.5 py-1.5 text-center font-sans text-[9.5px] uppercase tracking-architect transition-colors duration-200 ease-architect ${
                  range === val ? "bg-espresso-900 text-alabaster-50" : "bg-white text-taupe-500 hover:bg-alabaster-50 hover:text-espresso-900"
                }`}
              >
                {label}
              </button>
      ...
      <div className="mx-auto max-w-6xl space-y-14 px-4 pt-7 sm:px-8 sm:pt-9">
        ...
        {/* Metric ledger */}
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-panel bg-taupe-200/60 shadow-riser ring-1 ring-taupe-200/50 sm:grid-cols-3 lg:grid-cols-6">
        ...
        {tab === "overview" && (
          <>
                ...
                <div className="panel p-5 sm:p-7">
                  <div className="relative pt-7">
                    <div aria-hidden className="pointer-events-none absolute inset-x-0 bottom-0 top-7 flex flex-col justify-between">
                      <span className="h-px w-full bg-taupe-200/70" />
                      <span className="h-px w-full bg-taupe-200/70" />
                      <span className="h-px w-full bg-taupe-200/70" />
                    </div>
                    <div className="relative flex h-32 items-end gap-px border-b border-taupe-300">
                      {dailyEntries.map(([day, count]) => (
                        <div key={day} className="group relative flex h-full flex-1 flex-col items-center justify-end">
                          <div className="pointer-events-none absolute -top-6 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded-edge bg-espresso-900 px-2 py-1 font-sans text-[9px] tracking-architect text-alabaster-50 opacity-0 transition-opacity duration-200 ease-architect group-hover:opacity-100">
                            {fmtDay(day)}: {count}
                          </div>
                          <div className="min-h-[2px] w-full bg-espresso-800 transition-colors duration-200 ease-architect group-hover:bg-brass-500" style={{ height: `${(count / maxDaily) * 100}%` }} />
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="mt-2.5 flex justify-between font-sans text-[9.5px] uppercase tracking-architect text-taupe-400">
            ...
            <div className="grid gap-9 lg:grid-cols-2">
              ...
              <section className="animate-rise">
                ...
                <div className="space-y-2.5">
                  {[
                    { label: "Site Visitors", count: uniqueSessions, color: "bg-espresso-900" },
                    { label: "Viewed Estimator", count: pageCounts["/estimate"]?.sessions.size || 0, color: "bg-espresso-700" },
                    { label: "Viewed Consultation", count: pageCounts["/consultation"]?.sessions.size || 0, color: "bg-espresso-500" },
                    ...
                    <div key={stage.label} className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
                      ...
                      <div className="h-6 flex-1 overflow-hidden border border-taupe-200/60 bg-alabaster-100">
                        <div
              ...
              <section className="animate-rise">
                ...
                <div className="panel p-5 sm:p-7">
                  <div className="relative pt-7">
                    ...
                    <div className="relative flex h-28 items-end gap-px border-b border-taupe-300">
                      {hourlyCounts.map((count, h) => (
                        <div key={h} className="group relative flex h-full flex-1 flex-col items-center justify-end">
                          ...
                          <div
                            className={`min-h-[2px] w-full transition-colors duration-200 ease-architect ${count > 0 ? "bg-espresso-700 group-hover:bg-brass-500" : "bg-taupe-200/80"}`}
            ...
            <section className="animate-rise">
              ...
              <div className="max-h-80 overflow-y-auto border-t border-taupe-200/70">
                {filtered.slice(0, 40).map((v) => (
                  <div key={v.id} className="flex items-center gap-3 border-b border-taupe-200/55 py-2.5 transition-colors duration-200 ease-architect last:border-0 hover:bg-white">
                    <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-edge border font-sans text-[10px] ${
                      v.device === "iOS" ? "border-espresso-900 bg-espresso-900 text-alabaster-50" : v.device === "Android" ? "border-patina-200 bg-patina-50 text-patina-700" : v.device === "Desktop" ? "border-taupe-200 bg-alabaster-100 text-taupe-600" : "border-taupe-200/60 bg-white text-taupe-400"
                    }`}>
                      {(v.device || "?")[0]}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[13px] text-espresso-900">
                        <span>{pageLabels[v.page] || v.page}</span>
                      </p>
                      <div className="mt-0.5 flex items-center gap-2 font-sans text-[9.5px] uppercase tracking-architect text-taupe-400">
                        <span>{v.browser}</span>
                    ...
                    <span className="shrink-0 whitespace-nowrap font-sans text-[9.5px] uppercase tracking-architect tabular-nums text-taupe-400">{timeAgo(v.created_at)}</span>
                  </div>
                ))}
                {filtered.length === 0 && (
                  <div className="blueprint-grid px-8 py-16 text-center">
                    <p className="display-sm">No page views</p>
                    <p className="mx-auto mt-2 max-w-xs text-[13px] leading-relaxed text-taupe-500">Nothing was recorded in this period.</p>
                  </div>
                )}
              </div>
            </section>
              ...
              <div className="panel overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[520px] text-left">
                    ...
                    <tbody>
                      {pageEntries.map((p) => (
                        <tr key={p.page} className="border-b border-taupe-200/50 transition-colors duration-200 ease-architect last:border-b-0 hover:bg-alabaster-50">
                          <td className="px-5 py-4 sm:px-5">
                            <p className="text-[13px] font-medium tracking-[-0.01em] text-espresso-900">{pageLabels[p.page] || p.page}</p>
                            <p className="mt-0.5 font-sans text-[10px] text-taupe-400">{p.page}</p>
                          </td>
                          <td className="figure px-5 py-4 text-right text-[13px]">{p.views}</td>
                          <td className="px-5 py-4 text-right text-[13px] tabular-nums text-taupe-500">{p.sessions}</td>
                          <td className="px-5 py-4 text-right sm:px-5">
                            <div className="inline-flex items-center gap-2.5">
                              <div className="h-1 w-16 overflow-hidden bg-alabaster-200">
                                <div className="h-full bg-espresso-800" style={{ width: `${totalViews > 0 ? (p.views / totalViews) * 100 : 0}%` }} />
                              </div>
                              <span className="w-8 text-right font-sans text-[10px] tabular-nums text-taupe-500">
            ...
            <section className="animate-rise">
              ...
              <div className="border-t border-taupe-200/70">
                {entryEntries.slice(0, 10).map(([page, count]) => (
                  <div key={page} className="flex items-baseline justify-between gap-4 border-b border-taupe-200/55 py-3 transition-colors duration-200 ease-architect hover:bg-white">
                    <div className="min-w-0">
                      <span className="text-[13px] font-medium tracking-[-0.01em] text-espresso-900">{pageLabels[page] || page}</span>
                      <span className="ml-2 font-sans text-[10px] text-taupe-400">{page}</span>
                    </div>
                ...
                {entryEntries.length === 0 && (
                  <p className="py-8 text-center font-sans text-[10px] uppercase tracking-architect text-taupe-400">No data</p>
                )}
              </div>
            </section>
          </>
        ...
        {tab === "flow" && (
          <>
            <section className="animate-rise">
              ...
              <p className="-mt-1 mb-4 text-[12.5px] leading-relaxed text-taupe-500">
              ...
              <div className="border-t border-taupe-200/70">
                {flowEntries.map(([flow, count]) => {
                  ...
                  return (
                    <div key={flow} className="flex flex-col gap-2.5 border-b border-taupe-200/55 py-3 transition-colors duration-200 ease-architect hover:bg-white sm:flex-row sm:items-center sm:gap-4">
                      <div className="flex min-w-0 flex-1 items-center gap-2">
                        <span className="max-w-[45%] truncate rounded-edge border border-taupe-200 bg-alabaster-100 px-2.5 py-1 font-sans text-[10px] uppercase tracking-architect text-taupe-600 sm:max-w-[160px]">
                        ...
                        <span className="max-w-[45%] truncate rounded-edge border border-brass-200 bg-brass-50 px-2.5 py-1 font-sans text-[10px] uppercase tracking-architect text-brass-600 sm:max-w-[160px]">
                          {pageLabels[to] || to}
                        </span>
                      </div>
                      <div className="flex shrink-0 items-center gap-2.5">
                        <div className="h-1 w-20 overflow-hidden bg-alabaster-200 sm:w-16">
                          <div className="h-full bg-espresso-800" style={{ width: `${(count / (flowEntries[0]?.[1] || 1)) * 100}%` }} />
                        </div>
                ...
                {flowEntries.length === 0 && (
                  <div className="blueprint-grid px-8 py-16 text-center">
                    ...
                    <p className="mx-auto mt-2 max-w-xs text-[13px] leading-relaxed text-taupe-500">Multi-page visits are needed to map a path.</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section className="animate-rise">
              ...
              <div className="grid grid-cols-2 gap-px overflow-hidden rounded-panel bg-taupe-200/60 shadow-riser ring-1 ring-taupe-200/50 sm:grid-cols-4">
                {[
                  ...
                  <div key={d.label} className="bg-white px-4 py-5 transition-colors duration-300 ease-architect hover:bg-alabaster-50 sm:px-5">
        ...
        {tab === "sources" && (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="panel p-5 sm:p-7">
              <div className="mb-3 flex items-baseline justify-between gap-4 border-b border-taupe-200/70 pb-2.5">
                ...
                {(() => {
                  ...
                  return Object.entries(screenCounts).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([s, c]) => (
                    <div key={s} className="flex items-baseline justify-between gap-3 py-2">
                      <span className="min-w-0 truncate font-sans text-[11px] tabular-nums text-taupe-600">{s}</span>
...
function StatCard({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="bg-white px-5 py-5 transition-colors duration-300 ease-architect hover:bg-alabaster-50 sm:px-5 sm:py-5">
      <p className="eyebrow">{label}</p>
      <p className={`figure mt-2 truncate text-[1.5rem] leading-none sm:text-[1.875rem] ${color || "text-espresso-900"}`}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="panel p-5 sm:p-7">
      ...
      <div className="divide-hairline">
        {entries.slice(0, 8).map(([key, count]) => (
          <div key={key} className="flex items-baseline justify-between gap-3 py-2">
            <div className="flex min-w-0 items-baseline gap-2">
              ...
              <span className="truncate text-[12.5px] text-espresso-900">{key}</span>
            </div>
        ...
        {entries.length === 0 && (
          <p className="py-3 font-sans text-[10px] uppercase tracking-architect text-taupe-400">No data</p>
        )}
      </div>
    </div>
  );
...
function deviceColor(d: string): string {
  if (d === "iOS") return "bg-espresso-900";
  if (d === "Android") return "bg-patina-600";
  if (d === "Desktop") return "bg-brass-500";
  if (d === "Tablet") return "bg-taupe-400";
  return "bg-alabaster-300";
}

...
export default function EstimateDetailPage() {
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-sans text-[13px] tracking-architect text-ink-500">Loading estimate</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pb-28 text-left">
      ...
      {/* ── Sheet header ──────────────────────────────────────────────── */}
      <div className="border-b border-rule-300/70 bg-paper-50">
        <div className="mx-auto max-w-4xl px-4 py-6 sm:px-8 sm:py-8">
          <button
            type="button"
            onClick={() => router.push("/admin/projects")}
            className="btn-quiet -ml-3 mb-5 font-sans text-[13px] tracking-architect"
          >
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
            {estimate.estimate_number && (
              <span className="font-sans text-[10px] tracking-architect text-ink-500">{estimate.estimate_number}</span>
            )}
          ...
          <h1 className="display-lg mt-2.5 truncate">
            {estimate.name || "No name on file"}
          </h1>
          <p className="mt-2 font-sans text-[13px] tracking-architect text-ink-500">{estimate.project_type}</p>
      ...
      <div className="mx-auto max-w-4xl space-y-14 px-5 pt-12 sm:px-8 sm:pt-16">
        ...
        {estimate.estimate_number && (
          <section className="animate-rise">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
              <span className="rounded-edge border border-rule-300 bg-paper-50 px-3 py-1.5 font-sans text-[11.5px] tracking-architect text-ink-900">
                {estimate.estimate_number}
              </span>
              ...
              {proposalNumber ? (
                <span className="rounded-edge border border-forest-200 bg-forest-50 px-3 py-1.5 font-sans text-[11.5px] tracking-architect text-forest-700">
                  {proposalNumber}
                </span>
              ) : (
                <span className="text-[12.5px] leading-relaxed text-ink-500">
                  Becomes{" "}
                  <span className="font-sans text-[11.5px] tracking-architect text-ink-900">
        ...
        {!isConverted && (
          <section className="animate-rise">
            ...
            <div className="grid grid-cols-1 gap-px overflow-hidden rounded-edge bg-paper-200 shadow-riser ring-1 ring-rule-300/60 sm:inline-grid sm:grid-cols-3">
              {(["new", "contacted", "consultation_scheduled"] as const).map((s) => (
                <button
                  key={s}
                  type="button"
                  onClick={() => updateStatus(s)}
                  className={`px-6 py-3 text-center font-sans text-[13px] tracking-architect transition-colors duration-200 ease-architect ${
                    estimate.status === s
        ...
        <section className="animate-rise">
          <div className="title-block">
            ...
            <button
              type="button"
              onClick={openEditor}
              className="btn-outline shrink-0 px-3 py-1.5 font-sans text-[13px] tracking-architect"
            >
              <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <dl className="border-t border-rule-300/70">
            <div className="flex flex-col gap-0.5 border-b border-rule-300/55 py-3 sm:flex-row sm:items-baseline sm:gap-6">
              <dt className="eyebrow sm:w-44 sm:shrink-0">Name</dt>
              <dd className="min-w-0 text-[13.5px] text-ink-900">{estimate.name || "Not provided"}</dd>
            </div>
            <div className="flex flex-col gap-0.5 border-b border-rule-300/55 py-3 sm:flex-row sm:items-baseline sm:gap-6">
              <dt className="eyebrow sm:w-44 sm:shrink-0">Email</dt>
              <dd className="min-w-0 break-words text-[13.5px] text-ink-900">
                {estimate.email ? (
                  <a href={`mailto:${estimate.email}`} className="underline decoration-ink-300/20 underline-offset-4 transition-colors duration-200 ease-architect hover:decoration-ink-300">{estimate.email}</a>
                ) : (
            ...
            <div className="flex flex-col gap-0.5 border-b border-rule-300/55 py-3 sm:flex-row sm:items-baseline sm:gap-6">
              ...
              <dd className="min-w-0 text-[13.5px] text-ink-900">
                {estimate.phone ? (
                  <a href={`tel:${estimate.phone}`} className="tnum underline decoration-ink-300/20 underline-offset-4 transition-colors duration-200 ease-architect hover:decoration-ink-300">{estimate.phone}</a>
                ) : (
                  <span className="text-ink-500">Not provided</span>
                )}
              </dd>
            ...
            <div className="flex flex-col gap-0.5 border-b border-rule-300/55 py-3 sm:flex-row sm:items-baseline sm:gap-6">
              ...
              <dd className="min-w-0 text-[13.5px] tabular-nums text-ink-900">{createdDate}</dd>
            </div>
            <div className="flex flex-col gap-0.5 border-b border-rule-300/55 py-3 sm:flex-row sm:items-baseline sm:gap-6">
              <dt className="eyebrow sm:w-44 sm:shrink-0">Project Address</dt>
              <dd className={`min-w-0 text-[13.5px] ${fullAddress ? "text-ink-900" : "text-ink-500"}`}>
          ...
          {estimate.notes && (
            <div className="panel-sunken mt-5 p-5 sm:p-7">
              <p className="eyebrow">
                Internal Notes <span className="normal-case tracking-normal text-ink-400">(never shown to the customer)</span>
              </p>
              <p className="mt-2 whitespace-pre-wrap text-[13px] leading-relaxed text-ink-500">{estimate.notes}</p>
            </div>
          )}
        </section>
        ...
        <section className="animate-rise">
          ...
          <dl className="border-t border-rule-300/70">
            <div className="flex flex-col gap-0.5 border-b border-rule-300/55 py-3 sm:flex-row sm:items-baseline sm:gap-6">
              ...
              <dd className="min-w-0 text-[13.5px] text-ink-900">{estimate.project_type}</dd>
            </div>
            <div className="flex flex-col gap-0.5 border-b border-rule-300/55 py-3 sm:flex-row sm:items-baseline sm:gap-6">
              <dt className="eyebrow sm:w-44 sm:shrink-0">Finish Level</dt>
              <dd className="min-w-0 text-[13.5px] capitalize text-ink-900">{estimate.scope_level}</dd>
            </div>
            <div className="flex flex-col gap-0.5 border-b border-rule-300/55 py-3 sm:flex-row sm:items-baseline sm:gap-6">
              <dt className="eyebrow sm:w-44 sm:shrink-0">Size / ZIP</dt>
              <dd className="min-w-0 text-[13.5px] tabular-nums text-ink-900">
          ...
          <div className="mt-5">
            ...
            <p className="panel-sunken mt-2 p-4 text-[13px] leading-relaxed text-ink-500 sm:p-5">
        ...
        <section className="animate-rise">
          ...
          <div className="panel overflow-hidden">
            <div className="border-b border-rule-300/70 bg-paper-50 px-6 py-5 text-ink-900">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="shrink-0 sm:text-right">
                  <p className="figure text-[15px] text-ink-900 sm:text-[16.5px]">
              ...
              <table className="w-full min-w-[420px] text-left">
                ...
                <tbody>
                  {(ed.line_items || []).map((item: any, i: number) => (
                    <tr key={i} className="border-b border-rule-300/50 transition-colors duration-200 ease-architect last:border-b-0 hover:bg-paper-100">
                      <td className="px-5 py-5 align-top font-sans text-[10px] tabular-nums text-ink-400">
                        {i + 1}
                      </td>
                      <td className="px-2 py-3.5 align-top">
                        <p className="text-[13.5px] font-medium leading-snug tracking-[-0.01em] text-ink-900">{item.item}</p>
                        {item.notes && <p className="mt-1 text-[12px] leading-relaxed text-ink-500">{item.notes}</p>}
                      </td>
                ...
                <tfoot>
                  <tr className="border-t border-rule-300 bg-paper-50/60">
        ...
        {reminderCount > 0 && (
          <section className="animate-rise">
            ...
            <ol className="border-t border-rule-300/70">
              {(estimate.reminder_emails as any[]).map((r: any, i: number) => (
                <li key={i} className="relative flex flex-col gap-1 border-b border-rule-300/55 py-3 pl-6 sm:flex-row sm:items-baseline sm:justify-between sm:gap-6">
                  <span aria-hidden className="absolute left-0 top-[19px] h-px w-3.5 bg-rule-300" />
                  <span aria-hidden className="absolute left-[13px] top-[16px] h-[7px] w-[7px] rounded-full border border-rule-300 bg-paper-50" />
                  <span className="min-w-0 truncate text-[13px] text-ink-900">
                    Sent to <span className="text-ink-500">{r.to}</span>
                  </span>
                  <span className="shrink-0 font-sans text-[13px] tracking-architect tabular-nums text-ink-500">
        ...
        <section className="animate-rise">
          ...
          <div className="panel flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
            <div className="flex min-w-0 items-center gap-3">
              <svg aria-hidden className="h-4 w-4 shrink-0 text-ink-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate font-sans text-[11.5px] text-ink-900">wdocustom.com/estimate/{estimate.token}</p>
              </div>
            </div>
            <button
              type="button"
              ...
              className="btn-outline shrink-0 self-start px-4 py-2 font-sans text-[13px] tracking-architect sm:self-auto"
            >
              Copy
            </button>
          </div>
      ...
      {/* ── Edit customer sheet ───────────────────────────────────────── */}
      {isEditOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-paper-50/55 p-4 backdrop-blur-sm sm:items-center">
          <div className="my-8 w-full max-w-lg animate-rise rounded-sheet border border-rule-300 bg-paper-50 shadow-lift">
            <div className="border-b border-rule-300/70 px-6 py-5 sm:px-6">
            ...
            <div className="flex items-center justify-end gap-2 border-t border-rule-300/70 px-6 py-5 sm:px-6">
              <button
                type="button"
